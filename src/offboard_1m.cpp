...
bool is_reached(geometry_msgs::PoseStamped& pose,
    ...
    printf("Target z:%.2lf, current z:%.2lf\n", setpoint.pose.position.z, pose.pose.position.z);
    if (abs(pose.pose.position.x - setpoint.pose.position.x)<0.3 &&
        abs(pose.pose.position.y - setpoint.pose.position.y)<0.3 &&
        abs(pose.pose.position.z - setpoint.pose.position.z)<0.3){
        ROS_INFO("Reached point! deltaX=%.2lf, deltaY=%.2lf, deltaZ=%.2lf\n",
            abs(pose.pose.position.x - setpoint.pose.position.x),
            abs(pose.pose.position.y - setpoint.pose.position.y),
            abs(pose.pose.position.z - setpoint.pose.position.z));
        return true;
    }else{
        return false;
    }
