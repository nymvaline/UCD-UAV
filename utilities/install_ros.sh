...
rosinstall_generator ros_comm mavros --rosdistro jade --deps --wet-only --exclude roslisp --tar > jade-ros_comm-wet.rosinstall

echo "*** wstool ***"
sudo wstool init src -j1 jade-ros_comm-wet.rosinstall
while [ $? != 0 ]; do
...
rosdep install --from-paths src --ignore-src --rosdistro jade -y -r --os=debian:wheezy
...
echo "*** Building ROS ***"
sudo ./src/catkin/bin/catkin_make_isolated --install -DCMAKE_BUILD_TYPE=Release --install-space /home/ros/jade

sudo ln -sf /home/ros /opt/

echo "*** Updating .profile and .bashrc ***"
echo "source /home/ros/jade/setup.bash" >> ~/.profile
source ~/.profile
